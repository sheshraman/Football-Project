...
def transform_and_process_data(df):
    ...
    initial_transformer = joblib.load(
        'pipelines/20_05_2023-15_26_28_custom_attribute_object.pkl')
    df = initial_transformer.transform(df)

    transformer = joblib.load(
        'pipelines/20_05_2023-15_26_28_full_transformer.pkl')
    transformed_data = transformer.transform(df)

    model = joblib.load('models/20_05_2023-15_27_20_best_model.pkl')
    predicted_value = model.predict(transformed_data)

    # print('Predicted Value : ', predicted_value)

    target_transformer = joblib.load(
        'pipelines/20_05_2023-15_26_28_target_pipeline.pkl')
    actual_pred_value = np.round(
        target_transformer.inverse_transform(predicted_value.reshape(-1, 1)), 2)
...
def file_predict():
    ...
    return render_template('matched_players.html', output_dict=output_dict)

    '''
    position = request.form['position_file']

    output_dict = transform_and_process_data(player_data)
    # print('output_dict : ', output_dict)

    # Extracting pred_values from output_dict
    predicted_values = [player_info['pred_value']
                        for player_info in output_dict.values()]

    # read the "our_data" csv file
    our_data = pd.read_csv('my_players.csv')

   # Filter the rows in "our_data" based on predicted values and position
   # matching_rows = our_data[(our_data['pred_value'].isin(
   #     predicted_values)) & (our_data['position'] == position)]

    matching_rows = pd.DataFrame()

    for value in predicted_values:
        rows = our_data[((our_data['pred_value'] >= (value - 5)) &
                        (our_data['pred_value'] <= (value + 5))) & (our_data['position'] == position)]
    matching_rows = pd.concat([matching_rows, rows])

    # Convert the matching rows to a list of dictionaries
    matched_players = matching_rows.to_dict(orient='records')

    return render_template('output.html', matched_players=matched_players, predicted_values=predicted_values)
'''
...
def predict():
   ...
   #     predicted_values)) & (our_data['position'] == position)]
    ...
    for value in predicted_values:
        rows = our_data[((our_data['pred_value'] >= (value - 1)) &
                        (our_data['pred_value'] <= (value + 1))) & (our_data['position'] == position)]
    matching_rows = pd.concat([matching_rows, rows])
